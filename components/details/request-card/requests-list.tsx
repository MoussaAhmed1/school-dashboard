...
function PendingRequestsList({
  ...
  const [filteredRequests, setFilteredRequests] =
    useState<HistoryOfRequests[]>(_requests);
  // const [page, setPage] = useState(2);
  const approveRequestArray = useCallback(
    (requestId: string) => {
      const updatedRequestsArray = requests.filter(
  ...
  useEffect(() => {
    if (gradeId) {
      const filtered = requests.filter(
        (request) => request.grade?.id === gradeId,
      );
  ...
  useEffect(() => {
    ...
    // 🔹 Handle new requests
    const handleNewRequest = (request: HistoryOfRequests) => {
      if (status === request?.status) {
        const isExist = requests.find((req) => req.id === request?.id);
        if (isExist) {
          const updatedRequestsArray = requests.filter(
            (obj) => obj.id !== request.id,
          );
    ...
    // 🔹 Handle remove-request event
    const handleRemoveRequest = (data: {
      request_id: string;
      status: "PENDING" | "CONFIRMED";
    }) => {
      // Only act if the status matches the current tab
    ...
    socket.on(`new-request-${school_id}`, handleNewRequest);
    socket.on("remove-request", handleRemoveRequest);

    // Cleanup
    return () => {
      socket.off("connect", onConnect);
      socket.off("disconnect", onDisconnect);
      socket.off(`new-request-${school_id}`, handleNewRequest);
      socket.off("remove-request", handleRemoveRequest);
    };
  }, [requests, school_id, status]);
