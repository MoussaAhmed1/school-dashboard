...
export default function HistoryOfRequestsPage({ params }: paramsProps) {
  ...
  const breadcrumbItems = [{ title: t("historyOfRequests"), link: `/dashboard/history-of-requests` }];
  ...
  if (error) {
    return (
      <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
        <BreadCrumb items={breadcrumbItems} />
        <div className="flex items-start justify-between">
          <Heading title={`${t("historyOfRequests")} (0)`} />
        </div>
        <Separator />
  ...
  return (
    <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
      ...
      <div className="flex items-start justify-between">
        <Heading title={`${t("historyOfRequests")} (${totalRequests})`} />
      </div>
